...
CONNECTOR_SOURCE = Path(__file__).parents[1] / "elasticsearch_connector.py"
...
def test_legacy_parser_values_are_rejected_during_initialization():
    ...
    assert 'config.get("ingest_parser")' in source
    assert "Custom ingestion parsers are no longer supported" in source
